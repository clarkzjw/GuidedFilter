...
void GuidedFilterMono(Mat guide, Mat src, Mat dst, int radius, double eps);
void GuidedFilterColor(Mat guide, Mat src, Mat dst, int radius, double eps);

void GuidedFilter(Mat guide, Mat src, Mat dst, int radius, double eps)
{
	int depth = guide.channels();
	CV_Assert(depth == 1 || depth == 3);

	if (depth == 3)
		GuidedFilterColor(guide, src, dst, radius, eps);
	else if (depth == 1)
		GuidedFilterMono(guide, src, dst, radius, eps);
}
...
void GuidedFilterColor(Mat guide, Mat src, Mat dst, int radius, double eps)
{
	...
	int nch = guide.elemSize();
	int gnch = src.elemSize();
